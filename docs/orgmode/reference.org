...
| =packset island [--workspace WS] CUE= | the memories a cue activates: activation, seed mark, id, text |
...
| =GET /v1/activate?workspace=&q=&limit== | the cluster a cue activates: top hits as seeds, two hops along the links |
...
| =links= | peers by entity overlap, at most eight, chosen by relative-neighbourhood pruning |
