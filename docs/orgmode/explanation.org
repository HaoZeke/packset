...
0.300. Within noise for this reader and not a lever: the arithmetic the
temporal questions ask for is the reader's, and a small reader does not
do it whether the distances are marked or not. The framing stays as an
option (`--no-timeline` is the benchmark's own prompt) and the rows are
...
longer sees the old fact beside the new; the multi-hop rows stay hard, as
the benchmark's authors found for every system they measured (at most
0.28). Their retrieval baselines, with a much stronger reader, sit at
