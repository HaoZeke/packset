#!/usr/bin/env bash
# Build the packset Sphinx/Shibuya site: org → rst → html
# Usage (from repo root or this directory):
#   ./docs/build.sh
# Optional: PACKSET_DOC_EXPORTER=emacs|pandoc  PACKSET_DOC_VENV=path
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

VENV="${PACKSET_DOC_VENV:-$ROOT/.venv-docs}"
