...
impl Service {
    ...
    /// caller asked: the stage is a forward pass a candidate, and the
    /// default first-stage ranking is what a seat already gets.
    ///
    /// # Errors
    ///
    /// [`AtomError`] for a bad set name, else the store's.
    pub fn search(
        ...
        let projected = crate::milli::search(corpus, query, first_limit, &dir, scope);
    ...
    pub fn status(
        ...
        Ok(json!({
            ...
            // The measured second stage. Off is the default because it is
            // not free: LoCoMo paid three hours of CPU at eight cores for
            // 1536 questions over the top 20.
            "rerank": {
...
mod tests {
    ...
    fn status_counts_by_kind_and_names_the_home() {
        ...
        // The second stage is off unless the host asked. Depth is the window
        // the LoCoMo arm measured, so a seat comparing live search to that
        // table is comparing the same cut.
        assert_eq!(status["rerank"]["enabled"], json!(false));
        assert_eq!(status["rerank"]["depth"], json!(20));
    ...
    fn search_leaves_the_cross_encoder_off() {
        ...
        let found = svc.search("w", "reviews", 8, None, &panel, false).unwrap();
        assert_eq!(found["rerank"], json!("off"), "{found}");
        assert_eq!(found["hits"].as_array().map(Vec::len), Some(1));
        let empty = svc.search("w", "", 8, None, &panel, true).unwrap();
    ...
    fn a_requested_rerank_without_an_encoder_leaves_the_ranking() {
        ...
        let off = svc
            .search("w", "reviews search", 8, None, &panel, false)
            .unwrap();
        let on = svc
            .search("w", "reviews search", 8, None, &panel, true)
            .unwrap();
        unsafe {
            match old {
